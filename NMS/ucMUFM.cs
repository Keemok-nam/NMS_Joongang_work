...
namespace NMS
{
    public partial class ucMUFM : UserControl
    {
        public ucMUFM()
        {
            InitializeComponent();
        ...
        public void FmStInit(bool flagAction)
        {
            if (clsCommon.nmsGUIUser == "경춘선")
            {
                ...
                SetEnable(lblMuFmPower, flagAction);
                SetEnable(lblMuFmRf, flagAction);
                SetImage(pbMuFmPower, NMS.Properties.Resources.st_OFF);
                SetImage(pbMuFmRf, NMS.Properties.Resources.st_OFF);
                SetEnable(pbMuFmPower, flagAction);
                SetEnable(pbMuFmRf, flagAction);
            }
            else
            {
            }                        
        ...
        {
            if (!lblMuFmPower.Enabled) FmStInit(true);

            if (fmData.otherSt[0] == 0) SetImage(pbMuFmPower, NMS.Properties.Resources.st_Normal);
            else SetImage(pbMuFmPower, NMS.Properties.Resources.st_Error);

            if (fmData.otherSt[1] == 0) SetImage(pbMuFmRf, NMS.Properties.Resources.st_Normal);
            else SetImage(pbMuFmRf, NMS.Properties.Resources.st_Error);
        }
    }
}
